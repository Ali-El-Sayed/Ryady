...
interface IRemoteDataSource {
    ...
    suspend fun <T> fetchAddresses(token: String): Response<T>
...
class RemoteDataSource private constructor(private val client: ApolloClient) : IRemoteDataSource {
    ...
    override suspend fun <T> createCartWithLines(
        ...
        return when {
            (((response.data?.cartCreate?.userErrors?.size ?: -1) > 0)) -> {
                Response.Error(
                    response.data?.cartCreate?.userErrors?.first()?.message ?: "customer error == null"
                )
            }
    ...
    override suspend fun <T> fetchAddresses(token: String): Response<T> {
        val response = client.query(CustomerAddressQuery(token)).execute()
        return when {
            response.hasErrors() -> Response.Error(
                response.errors?.first()?.message ?: "Data Not Found"
            )

            else -> response.data?.customer?.addresses?.toAddressList().let {
                return Response.Success(it as T)
            }
    ...
    override suspend fun deleteAddress(token: String, addressId: String) {
        client.mutation(CustomerAddressDeleteMutation(addressId, token)).execute()
    }

    override suspend fun <T> createUserAddress(token: String, address: Address): Response<T> {
        val response = client.mutation(
        ...
        return when {
            response.hasErrors() -> Response.Error(
                response.errors?.first()?.message ?: "Error Saving the Address"
            )
